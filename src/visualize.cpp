...
void generateDotFile(const vector<pair<int, pair<int, int>>>& edges, const string& path, const string& filename) {
    ...
    file << "    edge [fontsize=50, penwidth=3.0];\n";  // Kenar yazılarını orta boy yap ve kenarları orta kalınlıkta yap
    
    // Tüm kenarları koyu gri yap ve kalınlaştır
    for (const auto& edge : edges) {
        file << "    " << edge.second.first << " -> " << edge.second.second 
             << " [label=\"" << edge.first << "\", color=\"#404040\", penwidth=3.0, fontsize=50];\n";
    }
    ...
    if (path != "NO SOLUTION") {
        ...
        for (size_t i = 0; i < path_nodes.size() - 1; ++i) {
            file << "    " << path_nodes[i] << " -> " << path_nodes[i + 1] 
                 << " [color=blue, penwidth=4.0, fontsize=50];\n";  // Path kenarlarının yazı boyutunu da orta seviyede tut
        }
    }
